...
export function generateNestedQuestions(orders, getId) {
  const questions = []

  const orderFieldGenerators = [
  ...
  const customerOrders = orders.map((_, i) => orders[i * SAMPLE_STRIDES.CUSTOMER_FIELD + 1] || orders[i]).filter(Boolean)
  questions.push(...rotateQuestions(
    customerOrders,
