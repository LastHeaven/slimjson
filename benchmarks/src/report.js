import { FORMATTER_DISPLAY_NAMES, QUESTION_TYPE_LABELS, QUESTION_TYPES } from './constants.js'
import { ACCURACY_DATASETS } from './datasets.js'
import { models, PRIMERS } from './evaluate.js'
import { supportsCSV } from './formatters.js'
import { generateQuestions } from './questions/index.js'
import { createProgressBar, tokenize } from './utils.js'

const EFFICIENCY_CHART_STYLE = 'horizontal'
...
function generateDatasetCatalog(datasets) {
...
function generateEfficiencyRankingReport(
  ...
  const slimjson = formatResults.find(r => r.format === 'slimjson')
  ...
  let summary = ''
  if (slimjson && json) {
    const slimjsonVsJson = `**${(slimjson.accuracy * 100).toFixed(1)}%** accuracy (vs JSON's ${(json.accuracy * 100).toFixed(1)}%)`
    const tokenSavings = `**${((1 - slimjson.totalTokens / json.totalTokens) * 100).toFixed(1)}% fewer tokens**`
    summary = `slimjson achieves ${slimjsonVsJson} while using ${tokenSavings}.`
  }
...
function generateDetailedAccuracyReport(
  ...
  const summaryComparison = generateSummaryComparison(slimjson, json)
...
function generateModelBreakdown(
  ...
  return modelNames.map((modelName, i) => {
    ...
    const formatLines = modelResults.map((result) => {
      ...
      const prefix = result.format === 'slimjson' ? '→ ' : '  '
...
function generateSummaryComparison(
  ...
  if (!slimjsonResult || !json)
    return ''

  return `
> [!TIP]
> slimjson achieves **${(slimjsonResult.accuracy * 100).toFixed(1)}% accuracy** (vs JSON's ${(json.accuracy * 100).toFixed(1)}%) while using **${((1 - slimjsonResult.totalTokens / json.totalTokens) * 100).toFixed(1)}% fewer tokens** on these datasets.
`.trim()
}
...
function generateDatasetBreakdown(
  ...
  return ACCURACY_DATASETS.map((dataset) => {
    ...
    const tableRows = datasetResults.slice(0, 7).map(result =>
